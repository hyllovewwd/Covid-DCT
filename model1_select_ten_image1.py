...
def handle_path():
    """ 得到文件的路径"""
    path_patient = []
    path_header = 'D:/项目/queue2/pCT'
    with open('tCT_1515', 'r') as f:
        list1 = f.readlines()
        for p in list1:
            p = eval(p)
            path_patient.append(p)
    back_path = []
    p_length=len(path_patient)
    for p in path_patient:
        path_=str(p[0]).split("/")[-1]
        path_li = []
        for n in range(0, p_length):
            path = path_header + "/" + path_
            path_li.append(path)
        back_path.append(path_li)
    return back_path
...
def get_image(image_paths, patient_no):
    """ 复制图片到指定路径"""
    target_path_header = "D:/项目/combine_image/tCT"
